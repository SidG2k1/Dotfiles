...
Ensure the work happens in an isolated workspace: create a worktree
(Claude Code's worktree isolation, or `git worktree add`) or verify the
existing one.
...
residual load-bearing findings surface to your human partner when you
...
Done! Tests verified — presenting integration options (open PR / merge / iterate).
```
