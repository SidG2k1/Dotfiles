...
| Menu bar icons still sit at the old spacing after `./install.sh` | AppKit reads `NSStatusItem*` when an app *creates* its status item, so every already-running app keeps the spacing it started with | the installer restarts SystemUIServer and ControlCenter itself, which covers the system-side items; quit and reopen any third-party menubar app, or log out. `bin/dotfiles-doctor --only macos` confirms the preference itself is set |
